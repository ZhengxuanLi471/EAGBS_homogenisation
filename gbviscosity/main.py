...
def _gb_viscosity_factor(edge_key, gb_viscosity_map):
    """
    Get viscosity factor for a grain boundary.
    
    Parameters
    ----------
    edge_key : tuple
        (i, j) grain boundary identifier
    gb_viscosity_map : dict or None
        Maps edge_key -> viscosity factor.
    
    Returns
    -------
    float : Viscosity factor for this boundary (default 1.0 if not in map)
    """
    if gb_viscosity_map is not None and edge_key in gb_viscosity_map:
        return gb_viscosity_map[edge_key]
    return 1.0


def _add_gb_terms(a, mesh, sym, contact_pairs, gb_viscosity_map, omega, sliding=True):
    ...
    for (i, j), (left_name, right_name) in contact_pairs.items():
        ...
        viscosity_factor = _gb_viscosity_factor((i, j), gb_viscosity_map)
        omega_weight = max(omega * viscosity_factor, 1e-14)
        term3 = (1 / omega_weight) * complexify(
            (-t_s_Im, t_s_Re),
            (r_s_Re, r_s_Im)
        )
        term4 = -complexify(
        ...
        cb.AddIntegrator(term1 + term2 + term3 + term4 + term5 if sliding else term1 + term2 + term4 + term5 + term3)
...
def _add_outer_terms(
    ...
    for key, data in outer_contact_pairs.items():
        ...
        maxdist_value = float(disp_norm + 1e-3)
        r_s_Re = sym.__dict__[f"r_{edge}_s_Re"]
        r_s_Im = sym.__dict__[f"r_{edge}_s_Im"]
        t_s_Re = sym.__dict__[f"t_{edge}_s_Re"]
        t_s_Im = sym.__dict__[f"t_{edge}_s_Im"]
...
def _assemble_bilinear_form(
    ...
    _add_gb_terms(a, mesh, sym, contact_pairs, gb_viscosity_map, omega, sliding=True)
    _add_gb_terms(a, mesh, sym, contact_pairs, gb_viscosity_map, omega, sliding=False)
    _add_outer_terms(
        a,
        mesh,
        sym,
    ...
    if solver == "direct":
        ...
        with TaskManager():
            ...
            inv = CGSolver(mat=a.mat, pre=c.mat,
                           printrates='\r', maxiter=50, tol=1e-12)
...
def solve_rve(spaces, mesh, contact_pairs, outer_contact_pairs,
    ...
    ----------
    gb_viscosity_map : dict or None
        Maps (i, j) grain boundary -> viscosity factor.
    """
